...
###
#
# Kate/Matroska Chapter & Subtitle Script
#
###
...
#to make a Matroska stream with Kate subpictures
...
#in this version, subtitles are all generated from a Kate config file

#chapters are embedded in the Mastroska stream.
...
class BoutRender(object):
	...
	def StartKate(self):
		self.kate = "kate {\n"
		self.K("defs {\n")
		#definitions for colours etc
		self.K('define macro t1c\t'+Tuple2Txt(Bout.Teams[0].Colour)+"\n")
		self.K('define macro t2c\t'+Tuple2Txt(Bout.Teams[1].Colour)+"\n")
		
		#define some text locations
		self.K('define region "headerl"
		self.K('define region "headerc"
		self.K('define region "headerr"
		self.K('define region "footerl"
		self.K('define region "footerc"
		self.K('define region "footerr"

		#and a fader in and out
		self.K('define motion "fadein" {' + "\n")
		self.K(" semantics text color a\n")
		self.K(" curve { linear 2 points { 0 255 } } for 500m\n")
		self.K(" }\n")
		self.K('define motion "fadeout" {' + "\n")
		self.K(" semantics text color a\n")
		self.K(" curve { linear 2 points { 255 0 } } for 500m\n")
		self.K(" }\n")

		self.K(" }\n") #close the defs
		
	def KateEvent(self,status):
		#unpack the status

		#add the left hand event (changes to Team 1's jammer status)
		self.K("event {\n")
		self.K(status.start + " --> " + status.end + "\n")
		self.K("text " + some text)
		self.K("style { font size "+fontsize+" }\n")
		self.K('region "footerl"'+"\n")
		self.K(' motion "fadein"' + "\n")
		self.K(' motion ' #static for rest of time until
		self.K(' motion "fadeout"' + "\n")

		#and the right hand event (changes to Team 2's jammer status)
		self.K("event {\n")
		self.K(status.start + " --> " + status.end + "\n")
		self.K("text " + some text)
		self.K("style { font size "+fontsize+" }\n")
		self.K('region "footerr"'+"\n")
		self.K(' motion "fadein"' + "\n")
		self.K(' motion ' #static for rest of time until
		self.K(' motion "fadeout"' + "\n")


	def KateScore(self,status):
		#unpack the status

		#add the event to kate stream
		self.K("event {\n")
		self.K(status.start + " --> " + status.end + "\n")
		self.K("text " + some text)
		self.K("style { font size "+fontsize+" }\n")
		self.K('region "headerc"'+"\n")
		#use bitmapped team logos in corners?
