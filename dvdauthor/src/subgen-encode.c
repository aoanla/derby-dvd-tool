...
//icptr = img;// use if call to imgfix() commented out
    ...
    offset0=subo;
    for( y=0; y<s->yd; y+=2 ) /* top field */
        dvd_encode_row(y,s->xd,icptr);

    offset1=subo;
    for( y=1; y<s->yd; y+=2 ) /* bottom field */
        dvd_encode_row(y,s->xd,icptr);
...
/* 
   ...
   The direct method of converting time to delay values is to multiply time in seconds
   by 90000/1024 and truncate the value.
